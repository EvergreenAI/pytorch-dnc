...
class DNC(nn.Module):
    ...
    def _layer_forward(self, input, layer, hx=(None, None), pass_through_memory=True):
        ...
        output, chx = self.rnns[layer](input.unsqueeze(1), chx)
        output = output.squeeze(1)

        # clip the controller output
        if self.clip != 0:
            output = T.clamp(output, -self.clip, self.clip)
    ...
    def forward(self, input, hx=(None, None, None), reset_experience=False, pass_through_memory=True):
        ...
        # else:
        max_length = 1  # input.size(1) if self.batch_first else input.size(0)
        ...
        output = T.cat([input[:, :], last_read], 0)
        ...
        for layer in range(self.num_layers):
            ...
            output, (chx, m, read_vectors) = \
                self._layer_forward(output, layer, (chx, m), pass_through_memory)
            ...
            if read_vectors is not None:
                ...
                output = T.cat([output, read_vectors], 0)
            else:
                output = T.cat([output, last_read], 0)
            # inputs = outs
        ...
        # Put it back to batch-first (i/a)
        # outputs = T.stack(inputs, 0)  # 1 if self.batch_first else 0)

        # if is_packed:
        #   outputs = pack(output, lengths)

        if self.debug:
            return output, (controller_hidden, mem_hidden, read_vectors), viz
        else:
            return output, (controller_hidden, mem_hidden, read_vectors)
